    ...
    if (!$error) {
        if (strlen($kod12) != 12 || !ctype_digit($kod12)) {
            ...
            if ($filename === "") {
                ...
                $normalFile = "output/" . $filename . "-NORMAL.bmp";
                $mirrorFile = "output/" . $filename . "-LUSTRO.bmp";
            }
    ...
    <section class="input">
        <fieldset>
            ...
            <form method="POST" action="">
                ...
                <label>
                    <input type="checkbox" name="gwarancja" checked <?= $addWarranty ? 'checked' : '' ?>>
                    Dodaj gwarancję
                </label><br>
    ...
    <?php if ($normalSrc && $mirrorSrc): ?>
        <section class="output">
            <div class="preview">
                ...
                <div class="buttons">
                    <a href="<?= $normalSrc ?>" download><button>Pobierz Normal</button></a>
                </div>
            </div>
            <div class="preview">
                <img src="<?= $mirrorSrc ?>" alt="lustro">
                <div class="buttons">
                    <a href="<?= $mirrorSrc ?>" download><button>Pobierz Lustro</button></a>
                </div>
            </div>
        </section>
    <?php endif; ?>
